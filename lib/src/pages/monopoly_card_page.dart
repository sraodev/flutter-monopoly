...
class _MonopolyCardPageState extends State<MonopolyCardPage> {
  ...
  Widget _buildPayeeButton(
      List<Monopoly> monopoly, int payerCardIndex, int payeeCardIndex) {
    return CustomIconButton(
      ...
      onTap: () {
        setState(() {
          double payerAmount = calculateValue(_minusKey, transactionAmount,
          ...
          if (payerAmount.isNegative) {
            ...
            double payeeAmount = calculateValue(_plusKey, transactionAmount,
                  ...
                  children: <Widget>[
                    CustomIconButton(
                      ...
                      onTap: () {
                        ...
                        transactionAmount = _textEditingController.text.isEmpty
                            ? 1
                            : double.parse(_textEditingController.text);
                        print(transactionAmount);
                        transactionAmount = calculateValue(
                            _multiplyKey, transactionAmount, Constants.MILLION);
                        setState(() {
                          _textEditingController.clear();
                          _textEditingController.text =
                    ...
                    CustomIconButton(
                      ...
                      onTap: () {
                        ...
                        transactionAmount = _textEditingController.text.isEmpty
                            ...
                            : double.parse(_textEditingController.text);
                        transactionAmount = calculateValue(
                            _multiplyKey, transactionAmount, Constants.KILO);
                        setState(() {
                          _textEditingController.clear();
                          _textEditingController.text =
                              CurrencyFormater.withSuffix(transactionAmount);
                    ...
                    CustomIconButton(
                      ...
                      onTap: () {
                        FocusScope.of(context).requestFocus(new FocusNode());
                        transactionAmount =
                            widget.monopoly[widget.card.index].cardBalance;
                        transactionAmount = calculateValue(
                            _plusKey, transactionAmount, 2 * Constants.MILLION);
                        setState(() {
                          widget.monopoly[widget.card.index].cardBalance =
                              transactionAmount;
                          _textEditingController.text =
                ...
                Row(
                  ...
                  children: <Widget>[
                    _buildPayeeButton(
                      widget.monopoly,
                      widget.card.index,
                      (MonopolyCard.PLAYER_RED.index == widget.card.index)
                          ? MonopolyCard.BANKER.index
                          : MonopolyCard.PLAYER_RED.index,
                    ),
                    _buildPayeeButton(
                      widget.monopoly,
                      widget.card.index,
                      (MonopolyCard.PLAYER_GREEN.index == widget.card.index)
                          ? MonopolyCard.BANKER.index
                          : MonopolyCard.PLAYER_GREEN.index,
                    ),
                    _buildPayeeButton(
                      widget.monopoly,
                      widget.card.index,
                      (MonopolyCard.PLAYER_BLUE.index == widget.card.index)
                          ? MonopolyCard.BANKER.index
                          : MonopolyCard.PLAYER_BLUE.index,
                    ),
                    _buildPayeeButton(
                      widget.monopoly,
                      widget.card.index,
                      (MonopolyCard.PLAYER_YELLOW.index == widget.card.index)
                          ? MonopolyCard.BANKER.index
                          : MonopolyCard.PLAYER_YELLOW.index,
                    ),
                  ],
                ),
              ]),
